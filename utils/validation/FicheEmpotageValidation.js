...
exports.schemaKeys = joi.object({
  ...
  ficheEmpotage_declarantId: joi.number().integer().allow(0),
  ficheEmpotage_dateVisite: joi.date().options({ convert: true }).allow(null).allow(''),
...
exports.findFilterKeys = joi.object({
  ...
  ...Object.fromEntries(
    keys.map(key => [key, joi.object({
      ...
      ficheEmpotage_declarantId: joi.alternatives().try(joi.array().items(),joi.number().integer(),joi.object()),
      ficheEmpotage_dateVisite: joi.alternatives().try(joi.array().items(),joi.date().options({ convert: true }),joi.object()),
